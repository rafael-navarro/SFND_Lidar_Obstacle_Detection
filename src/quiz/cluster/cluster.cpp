...
void helper_proximity(const std::vector<std::vector<float>>& points, int id, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
	processed[id] = true;
	...
	{
		if(!processed[index])
			helper_proximity(points, index, cluster, processed, tree, distanceTol);
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for(int i=0; i<points.size(); i++)
	{
		...
		helper_proximity(points, i, cluster, processed, tree, distanceTol);
